...
import { OperatorApiError, operatorApi } from "../operator-api";
import { useOperatorAuth } from "../operator-auth";
import type { OperatorAuthSession } from "../operator-auth-storage";

export default function OperatorFirstAccessPasswordPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const updatedSession = await operatorApi.post<OperatorAuthSession>("/api/operadores-auth/definir-nova-senha", {
        novaSenha,
        confirmacaoSenha,
      });
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.eyebrow}>Primeiro acesso operacional</div>
        <h1 style={styles.title}>Defina sua nova senha</h1>

        <div style={styles.infoBlock}>
          <div style={styles.infoLabel}>Operador</div>
          <div style={styles.infoValue}>{session?.operador.nome}</div>
          <div style={styles.infoMeta}>{session?.operador.matricula}</div>
        </div>
